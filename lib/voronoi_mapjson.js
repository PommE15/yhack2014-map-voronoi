...
var getImgUrlFromFlickr = function (name, coord, cb) {
    ...
    //var jsonpUrl =  "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20flickr.photos.search%20where%20api_key%3D%22"+api_key+"%22%20and%20text%3D%22" + encodeURIComponent(name) + "%22&format=json&diagnostics=true&tags=food&callback=" + cbname;
...
voronoiMap = function(map, url, initialSelections) {
    ...
    .domain([min, min*30, min*75, min*125, max])
        ...
        .style("fill", getColorFromPoint)
        .style("fill2", getFillURLFromPointName)
